...
function recordToRow(record) {
  ...
  const fieldMap = {
    'Scrape_Timestamp': record.scrape_timestamp || new Date().toISOString(),
    'County': record.county || record.County,
    'Booking_Number': record.booking_id || record.Booking_Number,
    'Person_ID': record.person_id || record.Person_ID || '',
    ...
    'ZIP': record.zipcode || record.ZIP || '',
    'Mugshot_URL': record.mugshot_url || record.Mugshot_URL,
    'Charges': record.charges_raw || record.Charges,
    'Bond_Amount': record.total_bond || record.Bond_Amount || '0',
    'Bond_Paid': record.bond_paid || record.Bond_Paid || 'NO',
    'Bond_Type': record.bond_type || record.Bond_Type || '',
    'Court_Type': record.court_type || record.Court_Type || '',
    'Case_Number': record.case_number || record.Case_Number,
    'Court_Date': record.court_date || record.Court_Date,
    'Court_Time': record.court_time || record.Court_Time || '',
    'Court_Location': record.court_location || record.Court_Location,
    'Detail_URL': record.source_url || record.Detail_URL,
    'Lead_Score': record.qualified_score || record.Lead_Score || 0,
    'Lead_Status': record.Lead_Status || (record.is_qualified ? 'Qualified' : 'Cold')
  };
