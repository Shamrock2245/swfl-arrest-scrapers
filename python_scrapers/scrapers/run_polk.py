#!/usr/bin/env python3
"""
Polk County Scraper Runner - Production Ready

Integrates polk_solver.py with the Python scraper infrastructure:
...
def convert_to_arrest_record(raw_data: dict) -> ArrestRecord:
    ...
    record = ArrestRecord(
        ...
        Arrest_Date=raw_data.get('Booking_Date', ''), # Fallback to Booking if Arrest missing
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"👮 Polk County Scraper - Production Runner")
    print(f"{'='*80}\n")
    
    # Get script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    solver_path = os.path.join(script_dir, 'polk_solver.py')
    
    # Run the solver
    print(f"📡 Running Polk solver (days_back={args.days_back})...")
    try:
    ...
    try:
        ...
        # Write to Polk tab
        stats = writer.write_records(scored_records, county="Polk")
        
        # Also log to ingestion log
        writer.log_ingestion("Polk", stats)
        
        print(f"\n{'='*80}")
        print(f"✅ Polk County Scraper Complete!")
        print(f"{'='*80}")
