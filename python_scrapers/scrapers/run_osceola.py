#!/usr/bin/env python3
"""
Osceola County Scraper Runner - Production Ready

Integrates osceola_solver.py with the Python scraper infrastructure:
...
def main():
    ...
    start_date = (datetime.now() - timedelta(days=args.days_back)).strftime('%m/%d/%Y')
    
    print(f"\n{'='*80}")
    print(f"🏛️  Osceola County Scraper - Production Runner")
    print(f"{'='*80}\n")
    
    # Get script directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    solver_path = os.path.join(script_dir, 'osceola_solver.py')
    
    # Run the solver
    print(f"📡 Running Osceola solver (days_back={args.days_back})...")
    try:
    ...
    try:
        ...
        # Write to Osceola tab
        stats = writer.write_records(scored_records, county="Osceola")
        
        # Also log to ingestion log
        writer.log_ingestion("Osceola", stats)
        
        print(f"\n{'='*80}")
        print(f"✅ Osceola County Scraper Complete!")
        print(f"{'='*80}")
