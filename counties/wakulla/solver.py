#!/usr/bin/env python3
"""
Wakulla County Solver

Small county south of Tallahassee.

Source: https://www.wcso.org
Platform: unknown
"""
...
try:
    ...
    sys.stderr.write("[WAKULLA] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "https://www.wcso.org"
HEADERS = {
...
def scrape_wakulla(days_back=7, max_pages=10):
    """Scrape Wakulla County inmate data."""
    sys.stderr.write(f"[WAKULLA] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[WAKULLA] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[WAKULLA] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Wakulla','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_wakulla(), indent=2))
