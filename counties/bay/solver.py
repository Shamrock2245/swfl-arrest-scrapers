...
Panama City. Mobile-optimized JS page.

Source: https://baysomobile.org
Platform: mobile-js
"""
...
try:
    ...
    sys.stderr.write("[BAY] Missing requests/bs4\n")
    sys.exit(1)
...
def scrape_bay(days_back=7, max_pages=10):
    """Scrape Bay County inmate data."""
    sys.stderr.write(f"[BAY] Starting → {BASE_URL}\n")
    ...
    sys.stderr.write(f"[BAY] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Bay','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_bay(), indent=2))
