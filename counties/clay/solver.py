...
P2C (Police-to-Citizen) platform. Direct jail inmates page.

Source: http://p2c.claysheriff.com/jailinmates.aspx
Platform: p2c
"""
...
try:
    ...
    sys.stderr.write("[CLAY] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "http://p2c.claysheriff.com/jailinmates.aspx"
HEADERS = {
...
def scrape_clay(days_back=7, max_pages=10):
    """Scrape Clay County inmate data."""
    sys.stderr.write(f"[CLAY] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[CLAY] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[CLAY] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Clay','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_clay(), indent=2))
