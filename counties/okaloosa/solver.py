#!/usr/bin/env python3
"""
Okaloosa County Solver

ArchonixX jail system with booking search + facility roster.

Source: http://www3.myokaloosa.com/ArchonixXJailPublic/Default.aspx
Platform: archonix
"""
...
try:
    ...
    sys.stderr.write("[OKALOOSA] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "http://www3.myokaloosa.com/ArchonixXJailPublic/Default.aspx"
HEADERS = {
...
def scrape_okaloosa(days_back=7, max_pages=10):
    """Scrape Okaloosa County inmate data."""
    sys.stderr.write(f"[OKALOOSA] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[OKALOOSA] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[OKALOOSA] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Okaloosa','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_okaloosa(), indent=2))
