...
Very small county. May be app-only.

Source: https://www.gulfsheriff.com
Platform: unknown
"""
...
try:
    ...
    sys.stderr.write("[GULF] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "https://www.gulfsheriff.com"
HEADERS = {
...
def scrape_gulf(days_back=7, max_pages=10):
    """Scrape Gulf County inmate data."""
    sys.stderr.write(f"[GULF] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[GULF] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[GULF] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Gulf','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_gulf(), indent=2))
