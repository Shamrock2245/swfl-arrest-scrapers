#!/usr/bin/env python3
"""
Jefferson County Solver

Small county (Monticello). May require login.

Source: https://www.jcso-fl.org
Platform: unknown
"""
...
try:
    ...
    sys.stderr.write("[JEFFERSON] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "https://www.jcso-fl.org"
HEADERS = {
...
def scrape_jefferson(days_back=7, max_pages=10):
    """Scrape Jefferson County inmate data."""
    sys.stderr.write(f"[JEFFERSON] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[JEFFERSON] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[JEFFERSON] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Jefferson','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_jefferson(), indent=2))
