#!/usr/bin/env python3
"""
Hernando County Solver

ASP.NET search form. A-Z name iteration.

Source: https://www.hernandosheriff.org/inmate-search
Platform: asp-net
"""
...
try:
    ...
    sys.stderr.write("[HERNANDO] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "https://www.hernandosheriff.org/inmate-search"
HEADERS = {
...
def scrape_hernando(days_back=7, max_pages=10):
    """Scrape Hernando County inmate data."""
    sys.stderr.write(f"[HERNANDO] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[HERNANDO] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[HERNANDO] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Hernando','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_hernando(), indent=2))
