#!/usr/bin/env python3
"""
Flagler County Solver

Tyler Tech/New World CAD. Slow but functional.

Source: https://nwwebcad.fcpsn.org
Platform: tyler-tech
"""
...
try:
    ...
    sys.stderr.write("[FLAGLER] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "https://nwwebcad.fcpsn.org"
HEADERS = {
...
def scrape_flagler(days_back=7, max_pages=10):
    """Scrape Flagler County inmate data."""
    sys.stderr.write(f"[FLAGLER] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[FLAGLER] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[FLAGLER] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Flagler','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_flagler(), indent=2))
