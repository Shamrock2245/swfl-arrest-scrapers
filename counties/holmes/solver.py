#!/usr/bin/env python3
"""
Holmes County Solver

Small county (Bonifay). Basic jail page.

Source: http://holmescosheriff.org/jail-division.html
Platform: html-list
"""
...
try:
    ...
    sys.stderr.write("[HOLMES] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "http://holmescosheriff.org/jail-division.html"
HEADERS = {
...
def scrape_holmes(days_back=7, max_pages=10):
    """Scrape Holmes County inmate data."""
    sys.stderr.write(f"[HOLMES] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[HOLMES] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[HOLMES] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Holmes','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_holmes(), indent=2))
