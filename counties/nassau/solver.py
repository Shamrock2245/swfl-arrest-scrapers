#!/usr/bin/env python3
"""
Nassau County Solver

Tyler Tech/New World inmate inquiry.

Source: https://dssinmate.nassauso.com/NewWorld.InmateInquiry/nassau
Platform: tyler-tech
"""
...
try:
    ...
    sys.stderr.write("[NASSAU] Missing requests/bs4\n")
    sys.exit(1)
...
def scrape_nassau(days_back=7, max_pages=10):
    """Scrape Nassau County inmate data."""
    sys.stderr.write(f"[NASSAU] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[NASSAU] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[NASSAU] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Nassau','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_nassau(), indent=2))
