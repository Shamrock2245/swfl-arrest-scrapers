#!/usr/bin/env python3
"""
Hardee County Solver

Small county (Wauchula). Roster page.

Source: https://www.hardeeso.com/inmate-roster
Platform: html-list
"""
...
try:
    ...
    sys.stderr.write("[HARDEE] Missing requests/bs4\n")
    sys.exit(1)

BASE_URL = "https://www.hardeeso.com/inmate-roster"
HEADERS = {
...
def scrape_hardee(days_back=7, max_pages=10):
    """Scrape Hardee County inmate data."""
    sys.stderr.write(f"[HARDEE] Starting → {BASE_URL}\n")
    ...
    try:
        ...
        sys.stderr.write(f"[HARDEE] FAIL: {e}\n")
        return []
    ...
    sys.stderr.write(f"[HARDEE] Total: {len(records)}\n")
    return records
...
def _parse(soup):
    ...
    if not records:
        for c in soup.find_all(['div','article','li'], class_=re.compile(r'inmate|roster|card|entry|booking|arrest', re.I)):
            ...
            if m:
                records.append({'County':'Hardee','State':'FL','Scrape_Timestamp':datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    'Last_Name':m.group(1),'First_Name':m.group(2),'Full_Name':f"{m.group(1)}, {m.group(2)}"})
    return records


if __name__ == "__main__":
    print(json.dumps(scrape_hardee(), indent=2))
